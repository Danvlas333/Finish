...
LICENSE_FILE = 'licenses.xlsx'
LICENSE_ACTIVATIONS_FILE = 'license_activations.json'
...
def load_activations():
    global license_activations
    try:
        if os.path.exists(LICENSE_ACTIVATIONS_FILE):
            with open(LICENSE_ACTIVATIONS_FILE, 'r', encoding='utf-8') as f:
                data = json.load(f)
                license_activations = {
                    key: datetime.fromisoformat(val) for key, val in data.items()
                }
        else:
            license_activations = {}
    except Exception as e:
        print(f"[WARN] Не удалось загрузить активации: {e}")
        license_activations = {}

def save_activations():
    try:
        serializable = {key: val.isoformat() for key, val in license_activations.items()}
        with open(LICENSE_ACTIVATIONS_FILE, 'w', encoding='utf-8') as f:
            json.dump(serializable, f, ensure_ascii=False, indent=2)
    except Exception as e:
        print(f"[ERROR] Не удалось сохранить активации: {e}")
...
def load_valid_licenses():
    global VALID_LICENSES, LICENSE_INFO
    try:
        if not os.path.exists(LICENSE_FILE):
            print(f"[WARN] Файл лицензий {LICENSE_FILE} не найден!")
            VALID_LICENSES = set()
            LICENSE_INFO = {}
            return

        df = pd.read_excel(LICENSE_FILE)
        required_cols = {'license_key', 'status'}
        if not required_cols.issubset(df.columns):
            raise ValueError(f"В Excel должны быть колонки: {required_cols}")
        ...
        for _, row in df.iterrows():
            ...
            LICENSE_INFO[key] = {'status': status, 'days': days}
            if status == 'active':
                valid_keys.add(key)

        VALID_LICENSES = valid_keys
        print(f"[INFO] Загружено {len(VALID_LICENSES)} активных лицензий")
    except Exception as e:
        print(f"[ERROR] Не удалось загрузить лицензии: {e}")
        VALID_LICENSES = set()
        LICENSE_INFO = {}

load_valid_licenses()

def activate_and_check_license(key):
    key = key.strip()
    if key not in VALID_LICENSES:
        return False, "Нет такого ключа доступа"

    info = LICENSE_INFO.get(key, {})
    days = info.get('days', 0)

    with activation_lock:
        now = datetime.now()

        if key in license_activations:
            activation_date = license_activations[key]
        else:
            license_activations[key] = now
            save_activations()
            activation_date = now
        ...
        if days > 0:
            ...
            return True, f"Действует ещё {remaining} дней"
        else:
            return True, "Бессрочная лицензия"

def is_valid_license(key):
    return key.strip() in VALID_LICENSES

def allowed_file(filename):
    return '.' in filename and filename.rsplit('.', 1)[1].lower() in ALLOWED_EXTENSIONS

# ======================
# Данные зонирования (без изменений)
# ======================
hsv_ranges = [
    ((0, 100, 100), (8, 255, 255)),
...
zone_info = [
    ...
    ("Зоны рекреационного назначения", "Р-1 Зона зеленых насаждений общего пользования (парков, скверов, садов, бульваров)"),
    ("Зоны рекреационного назначения", "Р-2 Зона размещения объектов отдыха и туризма (зона объектов рекреации)"),
    ("Зоны рекреационного назначения", "Р-3 Зона размещения объектов отдыха и туризма (зона объектов рекреации) вне границ населенных пунктов"),
    ...
    ("Производственные и коммунально-складские зоны", "П-1 Зона размещения производственных объектов I, II класса опасности"),
    ("Производственные и коммунально-складские зоны", "П-2 Зона размещения производственных объектов III класса опасности"),
    ("Производственные и коммунально-складские зоны", "П-3 Зона размещения производственных объектов IV, V классов опасности"),
    ...
    ("Производственные и коммунально-складские зоны", "П-5 Зона размещения производственных и коммунально-складских предприятий, расположенных вне границ населенных пунктов"),
    ("Производственные и коммунально-складские зоны", "П-5 Зона размещения коммунальных объектов и инженерной инфраструктуры (Янтарный)"),
    ...
    ("Зоны специального назначения", "Сп-2 Зона специального назначения, связанная с размещением государственных объектов"),
    ...
    ("Зоны сельскохозяйственного использования", "Сх-1 Зона сельскохозяйственных угодий в составе земель сельскохозяйственного назначения"),
    ("Зоны сельскохозяйственного использования", "Сх-2 Зона, занятая объектами сельскохозяйственного назначения и предназначенная для ведения сельского хозяйства, садоводства, личного подсобного хозяйства, развития объектов сельскохозяйственного назначения из земель сельскохозяйственного назначения"),
    ("Зоны сельскохозяйственного использования", "Сх-3 Зона сельскохозяйственного использования из земель населенных пунктов"),
    ("Зоны сельскохозяйственного использования", "Сх-4 Зона садоводческих или огороднических некоммерческих товариществ"),
    ("Зоны сельскохозяйственного использования", "Сх-5 Зона садоводческих или огороднических некоммерческих товариществ вне границ населенных пунктов"),
...
def process_zone_maps(img1_path, img2_path, output_dir):
    ...
    preview_full_path = os.path.join(output_dir, "preview_full.jpg")
    preview_full, _ = resize_image_for_display(cv2.imread(full_result_path), 2000, 2000)
    cv2.imwrite(preview_full_path, preview_full)
...
def verify_license():
    ...
    is_valid, message = activate_and_check_license(key)
    if is_valid:
...
def license_info():
    ...
    if not key or key not in VALID_LICENSES:
    ...
    with activation_lock:
        if key in license_activations:
            ...
            if days > 0:
                ...
                now = datetime.now()
                if now > expiry_date:
                    return jsonify({'status': 'expired', 'message': 'Срок действия истёк'})
                remaining = (expiry_date - now).days
                return jsonify({
                    'status': 'active',
                    'message': f'До окончания срока лицензии: {remaining} дней'
                })
            else:
                return jsonify({
                    'status': 'active',
            ...
            return jsonify({'status': 'unknown', 'message': 'Лицензия не активирована'})

@app.route('/compare', methods=['POST'])
def compare():
    license_key = request.form.get('license_key')
    if not license_key or not is_valid_license(license_key):
        return jsonify({'error': 'Неверный или отсутствующий лицензионный ключ'}), 403

    # Проверяем срок действия
    is_valid, _ = activate_and_check_license(license_key)
    if not is_valid:
        return jsonify({'error': 'Срок действия лицензии истёк'}), 403
